...
the contaminated commits were removed from this fork by a force-push
(local main was clean of them; the bad commits never made it past the
fork's main reference). They will be regenerated by the GPU pod. Do NOT
try to eval them from any older checkout — `git pull` first.
